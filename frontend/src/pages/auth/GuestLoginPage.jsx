...
function GuestLoginPage() {
  ...
  const handleVerifyRoom = async (e) => {
    ...
    try {
      ...
      await refreshParticipantList(inviteCode.trim(), roomData.id)
  ...
  useEffect(() => {
    ...
    const participantChannel = supabase
      .channel('room-universal-changes')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'room_guests' }, 
        (payload) => {
  ...
  const handleEnterRoom = async (e) => {
    ...
    try {
      ...
      const isDuplicateAtLastSecond = await checkRoomNicknameDuplicateApi(nickname.trim(), inviteCode.trim())

      if (isDuplicateAtLastSecond) {
        setIsAvailable(false)
        setMessage('❌ 앗! 방금 전 다른 유저가 이 닉네임을 먼저 사용했습니다. 다른 닉네임을 입력해 주세요.')
        return 
      }
  ...
  return (
    <section>
      ...
      {isRoomVerified && (
        <div>
          <button type="button" disabled={!isAvailable} onClick={handleEnterRoom}>
            이 방으로 최종 들어가기
          </button>
        </div>
      )}
      ...
      <div>
        <button type="button" onClick={() => navigate('/')}>처음 화면으로 돌아가기</button>
      </div>
    </section>
  )
}
