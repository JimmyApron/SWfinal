...
function VoteDetailPage() {
  ...
  const handleConfirm = async (option) => {
    ...
    try {
      await confirmVote(Number(voteid), option, Number(roomid), vote.votetype);
      await loadVote();
    } catch (error) {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#fff" }}>
      ...
      >
        <button
          onClick={() => navigate(`/rooms/${roomid}`, { state: { selectedTab: "vote" } })}
        ...
        {isCreator && !isEditMode ? (
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={() => setIsEditMode(true)} style={{ border: "none", background: "none", fontSize: "14px", color: "#555" }}>수정</button>
        ...
        {showVotingUI ? (
          <>
            {vote.voteoptions?.map((option) => (
              <label
                key={option.id}
                style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px", border: "1px solid #ddd", borderRadius: "8px", marginBottom: "10px", cursor: "pointer" }}
              >
                <input
                ...
                {option.optiontype === "date"
                  ? <span>{option.optiondate} / {option.starttime} ~ {option.endtime}</span>
            ...
            <button
              ...
              style={{ width: "100%", padding: "14px", backgroundColor: "#7c79ff", color: "#fff", border: "none", borderRadius: "8px", fontSize: "16px", cursor: "pointer" }}
            >
              투표하기
            </button>
          </>
                ...
                <div key={option.id} style={{ marginBottom: "16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", flex: 1 }}>
                      ...
                      {isConfirmed && (
                        <span style={{ fontSize: "12px", padding: "2px 8px", backgroundColor: "#7c79ff", color: "#fff", borderRadius: "10px" }}>
                          확정
                        </span>
                      )}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", whiteSpace: "nowrap" }}>
                      ...
                      <span
                        onClick={() => {
                          if (!vote.isanonymous) {
                            setShowVotersForOption(isShowingVoters ? null : option.id);
                          }
                        }}
                        style={{
                          color: "#7c79ff",
